...
- **Licence** : MIT
- **Extension Mail** : verifiee fonctionnelle apres desactivation du sandbox
...
### Session 5 (27 mars 2026)
- **Fix faux positifs sur mails transferes** : le sujet herite (Tr:, Fwd:, Re:) declenchait la detection a tort — desormais ignore dans ComposeSessionHandler.swift
- **Audit securite complet** des 7 fichiers Swift : aucune faille detectee (NSAttributedString HTML dans sandbox OK, pas d'injection possible, inputs sanitizes)
- **Bug corrige** : detection "De:/Objet:/Date:" dans MIMEParser testait les 3 sur une seule ligne (impossible) — remplace par detection de la ligne `De : xxx@...` (header forward Apple Mail)
- **Bug corrige** : precedence operateurs `||`/`&&` confuse dans extractNewPlainTextContent — separee en blocs clairs
- **Nettoyage code mort** : `showImportAlert` (ContentView, declare jamais utilise), variable `separator` (MIMEParser.parse, assignee jamais lue)
- **Optimisation** : KeywordDetector.findKeyword chargeait UserDefaults 2 fois (french + english) — reduit a 1 seul chargement
